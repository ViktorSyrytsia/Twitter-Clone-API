...
export class UsersRepository extends RepositoryBase<User> {
    ...
    public async findAll(
        ...
        const usersQuery: DocumentQuery<
            DocumentUser[],
            DocumentUser
    ...
    public async createUser(
        ...
        const newUser: DocumentUser = await this._repository.create(user);
    ...
    public async updateUser(
        ...
        const oldUser: DocumentUser = await this._repository.findById(
    ...
    public async changeAvatar(
        ...
        const updateUsert: DocumentUser = await this._repository
            .findByIdAndUpdate(
                userId,
                {
                    $set: { avatar: fileId, lastUpdated: Date.now() },
                },
                { new: true }
            )
            .select(this._selectFields)
    ...
    public async followUser(
        userIdToFollow: Types.ObjectId,
        principal: Principal
    ): Promise<DocumentUser> {
        const user: DocumentUser = await this._repository
            .findByIdAndUpdate(userIdToFollow, {
    ...
    public async unfollowUser(
        ...
        const user: DocumentUser = await this._repository
            .findByIdAndUpdate(userIdToUnFollow, {
    ...
    public async findUsersByUserIds(
        ...
        const findUsersQuery: DocumentQuery<
            DocumentUser[],
            DocumentUser
        > = this._repository.find({ _id: { $in: usersIds } });
        return this._addLazyLoadAndModify(
            findUsersQuery,
            principal,
            skip,
            limit
    ...
    public async findFollowers(
        ...
        const followerIds = (await this._repository.findById(userId)).followers,
            findUsersQuery: DocumentQuery<
                DocumentUser[],
                DocumentUser
            > = this._repository.find({ _id: { $in: followerIds } });
        return this._addLazyLoadAndModify(
            findUsersQuery,
            principal,
            skip,
    ...
    private async _addFields(
        ...
        if (
            principal &&
            (await principal.isAuthenticated()) &&
            !user._id.equals(principal.details._id)
        ) {
            user.isFollower = (
                await this._repository.findById(principal.details._id)
            ).followers.includes(user._id);
            user.isFollowed = (
                await this._repository.findById(user._id)
            ).followers.includes(principal.details._id);
        }
        user.avatar = await this._uploadsService.findFileById(
            user.avatar as Types.ObjectId
        );
        user.followersCount = user.followers.length;
        user.followingCount = await this._repository.countDocuments({
