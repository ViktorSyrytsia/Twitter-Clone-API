...
export class UploadsService {
    ...
    public async deleteFile(fileToRemove: DocumentFile): Promise<DocumentFile> {
        try {
            ...
            return await this._uploadsRepository.deleteFile(fileToRemove.id);
        } catch (error) {
            throw new HttpError(INTERNAL_SERVER_ERROR, error.message);
        }
    }
    ...
    private _pathCreator(fileTypeString: string, fileName: string): string {
        ...
        switch (fileType) {
            case 'image':
                path = `./public/files/images/${new Date().toISOString()}-${fileName}`;
                break;
            case 'audio':
                path = `./public/files/audios/${new Date().toISOString()}-${fileName}`;
                break;
            case 'video':
                path = `./public/files/videos/${new Date().toISOString()}-${fileName}`;
                break;
            default:
                path = `./public/files/others/${new Date().toISOString()}-${fileName}`;
                break;
        }
        return path;
    }
